...
export const changelog: ChangelogEntry[] = [
  {
    ...
    date: '2026-02-01',
    added: [
      '🎬 DISABLE_HERO_TRAILER环境变量：新增环境变量控制首页预告片播放,节省带宽和提升性能',
      '📊 System-Info API：新增 `/api/system-info` 端点,提供CPU、内存、进程和系统信息监控',
      '💬 弹幕样式配置：新增字号(12-48px)、速度(1-10)、透明度(0-100%)实时滑块控制',
      '⚖️ 视频源权重系统：实现视频源权重排序(0-100),默认权重50,高权重源优先显示',
      '🎌 Bangumi API优先：动漫内容优先使用Bangumi API,提升元数据质量',
      '🔍 智能搜索变体：自动生成搜索变体,支持并行搜索和智能去重',
    ],
    changed: [
      '⚡ 完整缓存体系：首页SWR缓存、视频LRU缓存、ACG搜索缓存、短剧API缓存',
