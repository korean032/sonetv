...
export async function getAvailableApiSites(user?: string): Promise<ApiSite[]> {
  ...
  if (!user) {
    // 按权重排序：权重高的排在前面（默认权重50）
    const sortedSites = sortByWeight(allApiSites);
    return applyVideoProxy(sortedSites, config);
  }
  ...
  if (userConfig.enabledApis && userConfig.enabledApis.length > 0) {
    ...
    const sortedSites = sortByWeight(userSites);
    return applyVideoProxy(sortedSites, config);
  }
  ...
  if (userConfig.tags && userConfig.tags.length > 0 && config.UserConfig.Tags) {
    ...
    if (enabledApisFromTags.size > 0) {
      ...
      const sortedSites = sortByWeight(tagSites);
      return applyVideoProxy(sortedSites, config);
    }
  }
 ...
 * 按权重排序视频源
 * @param sources 视频源列表
 * @returns 排序后的视频源列表（权重高的在前）
 */
function sortByWeight<T extends { weight?: number }>(sources: T[]): T[] {
  return sources.sort((a, b) => {
    const weightA = a.weight ?? 50; // 默认权重50
    const weightB = b.weight ?? 50;
    return weightB - weightA; // 降序排序，权重高的在前
  });
}
